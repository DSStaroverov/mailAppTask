let context, form, newLetterForm;
const letterUrl = "ajax/letter/";
...
function newLetter() {
    $("#newLetterTitle").html('Send Letter');
    form.find(":input").val("");
    $("#newLetterRow").modal();
}
...
function renderOpenLetterBtn(data, type, row) {
    if (type === "display") {
        return "<a onclick='openLetter(" + row.id + ");'><span class='fa fa-envelope-open'></span></a>";
    }
}